...
int main(int argc, char *argv[]) {
    ...
    char default_graph[256] = "../graph/mdual.graph";
	...
	Asyn_getSendBufferSize(&graph, size, rank, sendcounts);
	...
	for(int i=0;i<size;i++){
		read_buffersize_convergence(rank,i,allothersendcounts[i*(size+1)]);
	}
	i = size;
	...
	for(int i=0;i<size;i++)
	{
		write_buffer(i,rank,sdispls[i],sendcounts[i],sb);
	}
	
	for(int i=0;i<size;i++)
	{
		read_buffer(rank,i,rdispls[i],recvcounts[i],rb);
	}
	sort(rb, rb + rbsize, edgeComp); 
	updateGraph(&graph, rb, rbsize, rank);
        if (rbsize > 0) free(rb), rbsize = 0;
	if (sbsize > 0) free(sb), sbsize = 0;
	computing(rank, &graph, gmr, reduceResult); 
